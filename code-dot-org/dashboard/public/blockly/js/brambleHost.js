...
/******/ 	return __webpack_require__(__webpack_require__.s = 3841);
/******/ })
/************************************************************************/
/******/ ({
...
var base64 = __webpack_require__(1189)
var ieee754 = __webpack_require__(1190)
...
module.exports = function (url, dataType) {
  return _jquery2.default.ajax(url, { dataType: dataType });
};
...
module.exports = __webpack_require__(3842);
...
function ensureProjectRootDirExists(callback) {
  ...
  sh.mkdirp(weblabRoot + '/' + currentProjectPath, function (err) {
...
function putFilesInBramble(sources, callback) {
  ...
  sh.rm(weblabRoot + '/' + currentProjectPath, { recursive: true }, function (err) {
    // create project root directory
    sh.mkdirp(weblabRoot + '/' + currentProjectPath, function (err) {
      ...
      function writeSourceFile(currentIndex) {
        if (currentIndex < sources.files.length) {
          var _ret = function () {
            var file = sources.files[currentIndex];
            if (!file.name) {
...
function syncFilesWithBramble(fileEntries, currentProjectVersion, callback) {
  var fs = bramble_.getFileSystem();
  var Path = bramble_.Filer.Path;
  var Buffer = bramble_.Filer.Buffer;
  ...
  function writeFileEntry(i, callback) {
    if (i < fileEntries.length) {
      ...
      requestFileEntryAndWrite(fileEntries[i], function (err) {
        if (err) {
  ...
  function handleLocalChange(i, callback) {
    if (i < localChangeList.length) {
      (function () {
        var change = localChangeList[i];
        switch (change.operation) {
          case 'delete':
            webLab_.deleteProjectFile(change.file, function (err, versionId) {
              if (err) {
                callback();
              } else {
          ...
          case 'rename':
            webLab_.renameProjectFile(change.file, change.newFile, function (err, versionId) {
              if (err) {
                callback();
              } else {
                _lastSyncedVersionId = versionId;
          ...
          case 'change':
            getFileData(change.fileDataPath, function (err, fileData) {
              if (err) {
                callback();
              } else {
                webLab_.changeProjectFile(change.file, fileData, function (err, versionId) {
                  if (err) {
                    callback();
                  } else {
                    _lastSyncedVersionId = versionId;
  ...
  if (_lastSyncedVersionId !== currentProjectVersion) {
    ...
    removeAllFilesInBramble(function (err) {
      if (err) {
        callback();
      } else {
  ...
  sh.ls(weblabRoot + '/' + currentProjectPath, function (err, entries) {
    ...
    function getEntryFileData(i, callback) {
      if (i < entries.length) {
        (function () {
          var entry = entries[i];
          getFileData(entry.path, function (err, fileData) {
            if (err) {
              callback();
            } else {
              webLab_.changeProjectFile(entry.path, fileData, function (err, versionId) {
                if (err) {
                  callback();
                } else {
                  _lastSyncedVersionId = versionId;
...
function startInitialFileSync(callback, forceResetToStartSources) {
  var currentProjectVersion = webLab_.getCurrentFilesVersionId();
  ...
  if (!currentProjectVersion || forceResetToStartSources) {
    ...
    ensureProjectRootDirExists(function (err) {
      // Start the initial files sync
      syncFiles(callback);
    });
  }
...
function syncFiles(callback) {
  ...
  var currentFileEntries = webLab_.getCurrentFileEntries();
  var currentProjectVersion = webLab_.getCurrentFilesVersionId();
...
var brambleHost = {
  ...
  disableFullscreenPreview: disableFullscreenPreview,
...
function load(Bramble) {
  ...
  Bramble.once("ready", function (bramble) {
    ...
    function handleFileChange(path) {
      ...
      var hasExistingChangeForPath = _recentBrambleChanges.some(function (change) {
        return change.operation === 'change' && change.file === cleanedPath;
      });
    ...
    function handleFileRename(oldFilename, newFilename) {
      ...
      for (var i = 0; i < _recentBrambleChanges.length; i++) {
        var change = _recentBrambleChanges[i];
  ...
  Bramble.once("error", function (err) {
    ...
    // Send to New Relic
    _logToCloud2.default.addPageAction(_logToCloud2.default.PageAction.BrambleError, {
...
var PageAction = utils.makeEnum('DropletTransitionError', 'FirebaseRateLimitExceeded', 'SanitizedLevelHtml', 'UserJavaScriptError', 'RunButtonClick', 'StartWebRequest', 'StaticResourceFetchError', 'PegasusSectionsRedirect', 'BrambleError');
...
module.exports = {
   ...
   */
  addPageAction: function addPageAction(actionName, value, sampleRate) {
